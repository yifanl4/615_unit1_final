...
# Exploratory data analysis into the chemicals being applied to blueberries
...
```
After examining the commodities in the dataset, there are three types of berries: blueberries, raspberries, and strawberries. In this analysis, only the blueberries are focused and only the `Year` time period will be considered. 

## Data Cleaning and Organization

### Method
*
First filter out a dataset names `bberry` which contains all the blueberries data from the original dataset. Then separate the `Data Item` column into several different columns so that further cleaning process can be conducted. 
...
bberry %<>% rename(Avg = what, Marketing = meas, Harvest = label, Chem_family = DC_left_r, Materials = DC_right_r, Measures = units)
...
Below are the column names of the cleaned dataset for blue berries. 
```{r}
colnames(bberry)
```
Below is the preview of the cleaned dataset for blue berries. 
```{r}
kable(head(bberry, n=10)) %>% kable_styling(font_size=12)
```
## Visualizations

Suppose we want to explore the chemicals being applied to blueberries. First we need to filter out the related dataset named `unfood_1`. 
...
We first create a box plot of chemical values being applied to blueberries according to state. 
...
We can repeat the same procedure to visualize the distributions of chemicals being applied to blueberries according to different types of chemical. 
...
According to the box plot we can discover that there appears to have two different groups of chemicals with extremely different ranges of values. Thus we can divide our `unfood_1` dataset into two sets according to there two different groups of chemicals and then plot. 

```{r}
unfood_2 = filter(unfood_1, Chemical == c("FUNGICIDE", "HERBICIDE", "INSECTICIDE", "OTHER"))

unfood_3 = filter(unfood_1, Chemical == c("(NITROGEN)", "(PHOSPHATE)", "(POTASH)", "(SULFUR)"))
```
...
Then we can create a histogram to see the least and most variance chemicals. According to the histogram, we can see that nearly 300 out of 552 brands of blueberries have values of chemicals below 0.5. However, there is still a wide distribution of the values of chemicals contained by selected blueberries due to the varieties of chemicals which require further analysis. 
...
*
From the plot of chemical values according to state, we can tell that it appears New Jersey has the most various chemical values while Washington has the least various chemical values.
*
...
*
From the histogram of chemical values, we can tell that it appears most (nearly 300 out of 552) brands of blueberries contain chemical values below 0.5. 
